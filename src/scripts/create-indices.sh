...
# (mertalpt): Opensearch initizalizer script had to be patched to make it work with TLS.
# TODO (mertalpt): Look into 'https://datahubspace.slack.com/archives/CV2KB471C/p1722430574258409?thread_ts=1720515025.750159&cid=CV2KB471C' for a cleaner solution.
...
CURL_ARGS=(
  ...
  --cacert /ca-cert.pem  # (mertalpt): Original script does not account for TLS, we add this to fix it
)
...
function create_if_not_exists {
  ...
  RESOURCE_STATUS=$(curl "${CURL_ARGS[@]}" -vvv -o /dev/null -w "%{http_code}\n" "$ELASTICSEARCH_URL/$RESOURCE_ADDRESS")
