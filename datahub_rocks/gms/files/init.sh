...
sed -e "s/DATAHUB_DB_NAME/${DATAHUB_DB_NAME}/g" /datahub/postgres-setup/init.sql | tee -a /tmp/init-final.sql
...
if [ "${CDC_MCL_PROCESSING_ENABLED}" = "true" ]; then
    ...
    sed -e "s/DATAHUB_DB_NAME/${DATAHUB_DB_NAME}/g" \
        ...
        -e "s/CDC_PASSWORD/${CDC_PASSWORD}/g" /datahub/postgres-setup/init-cdc.sql > /tmp/init-cdc-final.sql
